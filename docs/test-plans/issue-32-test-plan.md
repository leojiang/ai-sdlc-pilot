...
| Refuses when `claude` is not on PATH, clear message | Fixture: temp git repo, PATH without `claude` — expect nonzero exit + message naming `claude`/PATH | automated — `make check-ai-review` |
